...
function PositionTwo() {
 ...
 /*  function handleClickCorrect(){
  ...
  return (
    <>
    <div className="positionTwoContainer">
    
        <div className='headAndReset'>
          <h1>Show the element <span style={{backgroundColor: "orange"}}>below</span></h1>
          <button onClick={handleReset}>Reset task</button>
        </div>
        <div className='position-container-two'>
          
        <div className="below-two"  onClick={handleClickCorrect} /* onClick={() => alert("SORRY, WRONG ANSWER :)")} */ >
        {isClickedCorrect && userHasClicked && <img src={smile} style={{width: "30px", height: "30px", border: "2px solid green", borderRadius: "50%"}}/>}
        </div>
        <div className="above-two" onClick={handleClickWrongTwo} /* onClick={() => alert("CORRECT ANSWER! WELL DONE! :)")} */ >
